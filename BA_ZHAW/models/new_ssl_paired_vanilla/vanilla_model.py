...
class VanillaModel(pl.LightningModule):
    def __init__(self, embed_dim, max_seq_length, device_, traV_embed_len, traJ_embed_len, trbV_embed_len, trbJ_embed_len, mhc_embed_len, hyperparameters):
        ...
        # Add the pretraining projection layer to match dimensions during pretraining
        physico_input_dim = 88  # Replace 88 with the actual dimension of the physicochemical features
        self.pretraining_projection = torch.nn.Linear(physico_input_dim * 3, self.classifier_in)

    def forward(self, epitope, tra_cdr3, trb_cdr3, v_alpha, j_alpha, v_beta, j_beta, mhc, is_pretraining=False):
        if is_pretraining:
            # Verwende alle physikochemischen Merkmale während des Vortrainings
            x = torch.cat([epitope, tra_cdr3, trb_cdr3], dim=1)
            # Projektion der physikochemischen Merkmale auf die erwartete Eingabedimension
            x = self.pretraining_projection(x)
    
            # Verwende eine zusätzliche Projektion, um die Dimension an den Zielwert anzupassen
            output = torch.nn.Linear(x.shape[1], epitope.shape[1]).to(self.device_)(x)
            return output
        else:
            # Originaler Pfad für das Haupttraining
            tra_epitope = torch.cat([tra_cdr3, epitope], dim=1)
            trb_epitope = torch.cat([trb_cdr3, epitope], dim=1)
    
            tra_v_embed = self.traV_embed(v_alpha.to(torch.long).to(self.device_)).unsqueeze(0).permute(1, 0, 2)
            trb_v_embed = self.trbV_embed(v_beta.to(torch.long).to(self.device_)).unsqueeze(0).permute(1, 0, 2)
            trb_j_embed = self.trbJ_embed(j_beta.to(torch.long).to(self.device_)).unsqueeze(0).permute(1, 0, 2)
            tra_j_embed = self.traJ_embed(j_alpha.to(torch.long).to(self.device_)).unsqueeze(0).permute(1, 0, 2)
            mhc_embed = self.mhc_embed(mhc.to(torch.long).to(self.device_)).unsqueeze(0).permute(1, 0, 2)
            ...
            x = concat_both_chains_flatten
    
        # Classifier zur Vorhersage der Bindungswahrscheinlichkeit
