...
class PadCollate:
    ...
    def pad_collate(self, batch):
        ...
        return {
            ...
            "task": task,
            "label": labels,
            "epitope_physico": torch.stack([torch.tensor(features, dtype=torch.float32) for features in epitope_physico]),
            "tra_physico": torch.stack([torch.tensor(features, dtype=torch.float32) for features in tra_physico]),
            "trb_physico": torch.stack([torch.tensor(features, dtype=torch.float32) for features in trb_physico])
        }
...
def main():
    ...
    physico_base_dir = f"../../data/physicoProperties/ssl_paired/{precision}" # physico data
    ...
    # PROJECT_NAME = f'dataset-{precision}_10x' # 10x
    PROJECT_NAME = f'dataset-{precision}' # BA
    ...
    print("erster Stopp")
    train_dataset = PairedVanilla(train_file_path, embed_base_dir,physico_base_dir, traV_dict, traJ_dict, trbV_dict, trbJ_dict, mhc_dict)
    print("erster zweiter Stopp")
    #train_dataset.print_sample()
    test_dataset = PairedVanilla(test_file_path, embed_base_dir,physico_base_dir, traV_dict, traJ_dict, trbV_dict, trbJ_dict, mhc_dict)
    print("erster dritter Stopp")
    #test_dataset.print_sample()
    val_dataset = PairedVanilla(val_file_path, embed_base_dir,physico_base_dir, traV_dict, traJ_dict, trbV_dict, trbJ_dict, mhc_dict)
    print("zweiter Stopp")
