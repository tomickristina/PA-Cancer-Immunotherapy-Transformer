...
class VanillaModel(pl.LightningModule):
    def __init__(self, embed_dim, max_seq_length, device_, traV_embed_len, traJ_embed_len, trbV_embed_len, trbJ_embed_len, mhc_embed_len, hyperparameters):
        ...
        # Physikochemische Embedding-Layer
        self.epitope_physico_layer = nn.Linear(88, embed_dim)
        self.tra_physico_layer = nn.Linear(88, embed_dim)
        self.trb_physico_layer = nn.Linear(88, embed_dim)
        ...
        # Define Classifier
        # Calculate classifier input size based on the known dimensions
        # Flattened output: 2 chains * (max_seq_length + 1 for epitope + 3 for v, j, mhc embeddings) * embedding dimension
        self.classifier_hidden = 64
        self.classifier = nn.Linear(self.n_hidden * 2 * max_seq_length, 1)
...
class VanillaModel(pl.LightningModule):
    ...
    def forward(self, epitope, tra_cdr3, trb_cdr3, v_alpha, j_alpha, v_beta, j_beta, mhc, epitope_physico, tra_physico, trb_physico):
        # Physikochemische Eingaben durch Embedding-Layer verarbeiten
        epitope_physico_embedded = self.epitope_physico_layer(epitope_physico)
        tra_physico_embedded = self.tra_physico_layer(tra_physico)
        trb_physico_embedded = self.trb_physico_layer(trb_physico)
        ...
        tra_epitope = torch.cat([tra_cdr3, epitope, tra_physico_embedded, epitope_physico_embedded], dim=1)
        trb_epitope = torch.cat([trb_cdr3, epitope, trb_physico_embedded, epitope_physico_embedded], dim=1)
        ...
        # Initialisierung des Klassifikators beim ersten Vorwärtsdurchlauf
        if self.classifier is None:
            self.classifier = nn.Linear(concat_both_chains_flatten.shape[1], 1).to(self.device_)

        logits = self.classifier(concat_both_chains_flatten)  # Dimension auf (Batch,) reduzieren
        return logits
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = self(epitope_embedding, tra_cdr3_embedding, trb_cdr3_embedding, v_alpha, j_alpha, v_beta, j_beta, mhc, epitope_physico, tra_physico, trb_physico).squeeze()
    ...
    def test_step(self, batch, batch_idx):
        ...
        output = self(epitope_embedding, tra_cdr3_embedding, trb_cdr3_embedding, v_alpha, j_alpha, v_beta, j_beta, mhc, epitope_physico, tra_physico, trb_physico).squeeze(1)
