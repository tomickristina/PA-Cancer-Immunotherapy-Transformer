...
class VanillaModel(pl.LightningModule):
    def __init__(self, embed_dim, max_seq_length, device_, traV_embed_len, traJ_embed_len, trbV_embed_len, trbJ_embed_len, mhc_embed_len, hyperparameters):
        ...
        Furthermore, this model DOES include physicochemical properties.
        """
        ...
        # Embedding dimension for allele-related embeddings
        self.allele_info_dim = 1024

        # MLP für physikochemische Eigenschaften
        self.epitope_physico_mlp = nn.Sequential(
            nn.Linear(88, 512),
            nn.ReLU(),
            nn.Linear(512, embed_dim)
        )
        self.tra_physico_mlp = nn.Sequential(
            nn.Linear(88, 512),
            nn.ReLU(),
            nn.Linear(512, embed_dim)
        )
        self.trb_physico_mlp = nn.Sequential(
            nn.Linear(88, 512),
            nn.ReLU(),
            nn.Linear(512, embed_dim)
        )

        # Embedding layers for allele-related features
       ...
       # Define Classifier
        ...
        self.classifier = Classifier(self.classifier_in, self.classifier_hidden, self.hyperparameters["dropout_linear"])
         ...
         # Attention block for combined features
        self.attention_block = nn.TransformerEncoderLayer(
            d_model=self.allele_info_dim, nhead=8, batch_first=True
        )

        # Classifier
        self.classifier = nn.Sequential(
            nn.Linear(self.allele_info_dim * 130, 512),
            nn.ReLU(),
            nn.Linear(512, 1)
        )


    def forward(self, epitope, tra_cdr3, trb_cdr3, v_alpha, j_alpha, v_beta, j_beta, mhc, epitope_physico, tra_physico, trb_physico):
        """
        Forward pass of the model.

        Args:
            epitope: Epitope embeddings
            tra_cdr3, trb_cdr3: TCR CDR3 sequence embeddings
            v_alpha, j_alpha, v_beta, j_beta: V/J segment IDs for TCR
            mhc: MHC embeddings
            epitope_physico, tra_physico, trb_physico: Physicochemical features

        Returns:
            logits: Output logits from the classifier
        """
        # Physikochemische Features verarbeiten
        epitope_physico_embedded = self.epitope_physico_mlp(epitope_physico)
        tra_physico_embedded = self.tra_physico_mlp(tra_physico)
        trb_physico_embedded = self.trb_physico_mlp(trb_physico)
    
        # Sicherstellen, dass physikochemische Merkmale dieselben Dimensionen haben
        epitope_physico_embedded = epitope_physico_embedded.unsqueeze(1)
        tra_physico_embedded = tra_physico_embedded.unsqueeze(1)
        trb_physico_embedded = trb_physico_embedded.unsqueeze(1)
    
        # Debug-Ausgaben
        print(f"tra_cdr3.shape: {tra_cdr3.shape}")
        print(f"epitope.shape: {epitope.shape}")
        print(f"tra_physico_embedded.shape: {tra_physico_embedded.shape}")
        print(f"epitope_physico_embedded.shape: {epitope_physico_embedded.shape}")
    
        # Kombinieren von TRA und Epitope
        tra_combined = torch.cat([tra_cdr3, epitope, tra_physico_embedded, epitope_physico_embedded], dim=1)
        trb_combined = torch.cat([trb_cdr3, epitope, trb_physico_embedded, epitope_physico_embedded], dim=1)
    
        # MHC-Features einbetten
        ...
        print(f"tra_combined.shape: {tra_combined.shape}")
        print(f"trb_combined.shape: {trb_combined.shape}")
        print(f"tra_v_embed.shape: {tra_v_embed.shape}")
        print(f"tra_j_embed.shape: {tra_j_embed.shape}")
        print(f"trb_v_embed.shape: {trb_v_embed.shape}")
        print(f"trb_j_embed.shape: {trb_j_embed.shape}")
        print(f"mhc_embed.shape: {mhc_embed.shape}")
    
        # Merkmale kombinieren
        tra_epitope_vj_mhc = torch.cat([tra_combined, tra_v_embed, tra_j_embed, mhc_embed], dim=1)
        trb_epitope_vj_mhc = torch.cat([trb_combined, trb_v_embed, trb_j_embed, mhc_embed], dim=1)
    
        # Multihead-Attention anwenden
        tra_attention = self.multihead_attn_global(tra_epitope_vj_mhc)
        trb_attention = self.multihead_attn_global(trb_epitope_vj_mhc)
    
        # Beide Chains kombinieren
        concat_both_chains = torch.cat([tra_attention, trb_attention], dim=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        tra_physico = batch["tra_physico"]
        trb_physico = batch["trb_physico"]
        ...
        output = self(
         ...
         # Entfernen der zusätzlichen Dimension
    ...
    def test_step(self, batch, batch_idx):
        # Extrahiere alle relevanten Eingaben aus dem Batch
        epitope_embedding = batch["epitope_embedding"]
        ...
        output = self(
            ...
            tra_physico,
            trb_physico
        ).squeeze(-1)  # Entferne zusätzliche Dimension
    
        # Sigmoid, um Wahrscheinlichkeiten zu erhalten
        ...
        # Vorhersagen, Labels, Sequenzen und Tasks speichern
        self.test_predictions.append(prediction.cpu())
        self.test_labels.append(label.cpu())
        self.test_tasks.append(task[0])  # Erster Task in der Batch-Dimension
        self.epitopes.append(epitope_sequence)
        self.tra_cdr3s.append(tra_cdr3_sequence)
        self.trb_cdr3s.append(trb_cdr3_sequence)
    
        return {"test_loss": test_loss, "predictions": prediction, "labels": label}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_loss", val_loss, prog_bar=True, batch_size=len(batch))
