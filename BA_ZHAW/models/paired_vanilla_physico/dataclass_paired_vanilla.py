...
class PairedVanilla(Dataset):
    def __init__(self, dataset_path, embed_base_path, physico_base_dir, traV, traJ, trbV, trbJ, mhc, transform=None):
        """
        ...
        # Physikochemische Merkmale Pfad
        self.epitope_physico_features_path = f"{physico_base_dir}/Epitope_physico.npz"
        self.tra_physico_features_path = f"{physico_base_dir}/TRA_CDR3_physico.npz"
        self.trb_physico_features_path = f"{physico_base_dir}/TRB_CDR3_physico.npz"
        ...
        # Physikochemische Merkmale laden
        epitope_physico_features = np.load(self.epitope_physico_features_path)
        tra_physico_features = np.load(self.tra_physico_features_path)
        trb_physico_features = np.load(self.trb_physico_features_path)
        ...
        # Physikochemische Merkmale mit den bestehenden Embeddings kombinieren
        self.data_frame["Epitope Physico"] = self.data_frame["Epitope"].map(epitope_physico_features)
        self.data_frame["TRA_CDR3 Physico"] = self.data_frame["TRA_CDR3"].map(tra_physico_features)
        self.data_frame["TRB_CDR3 Physico"] = self.data_frame["TRB_CDR3"].map(trb_physico_features)
    ...
    def __getitem__(self, index):
        ...
        epitope_physico = torch.tensor(row["Epitope Physico"], dtype=torch.float32)
        tra_physico = torch.tensor(row["TRA_CDR3 Physico"], dtype=torch.float32)
        trb_physico = torch.tensor(row["TRB_CDR3 Physico"], dtype=torch.float32)
